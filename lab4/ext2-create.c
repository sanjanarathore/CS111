...
void write_block_bitmap(int fd)
{
	...
	// 1 bit = 1 block -> 8 blocks per index
	// first 23 blocks used to set up file system
	// blocks 24-1023 are free
	// 1023 / 8 = 127r1
	for (int i = 0; i < 23; i++) {
       map_value[i / 8] |= (1 << (i % 8)); // Set the i-th bit to 1
    }
...
void write_inode_bitmap(int fd)
{
	...
	// inodes 1-13 used to set up file system
	// inodes 14-127 are free
	for (int i = 0; i < 13; i++) {
       map_value[i / 8] |= (1 << (i % 8)); // Set the i-th bit to 1
    }
...
void write_inode_table(int fd) {
	...
	// hello symbolic link pointing to hello world file
